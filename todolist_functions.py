...
def Data_insert(collection, data):
    ...
    collection.delete_many({})
    # 데이터 입력
    collection.insert_many(data)           # 1 hint    :  collection.insert_one(data)
...
def Todos(user_id, collection1, collection2):
    ...
    # todo중 하나 입력
    user_input = int(input("Title 번호: "))-1           # 5 hint   :   user_input = input("Title 번호: ")-1
    # Status 입력
    user_status = input("Status: ")           # 6 hint   :   user_status = int(input("Status: "))
...
# 종료 여부 입력 function
def End(collection, collection1, collection2):           # 7 hint :   def End(collection1, collection2):
                # 8 hint :   user_end = 'x'
    while True:
